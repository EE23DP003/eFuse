...
# 5 "C:/Users/Abhinav/.mchp_packs/Microchip/PIC16F1xxxx_DFP/1.8.149/xc8\\pic\\include\\builtins.h" 2 3
...
# 1 "./mcc_generated_files/clc4.h" 1
# 91 "./mcc_generated_files/clc4.h"
void CLC4_Initialize(void);
# 113 "./mcc_generated_files/clc4.h"
...
# 1 "./mcc_generated_files/clc2.h" 1
# 91 "./mcc_generated_files/clc2.h"
void CLC2_Initialize(void);
# 110 "./mcc_generated_files/clc2.h"
void CLC2_ISR(void);
# 131 "./mcc_generated_files/clc2.h"
...
# 44 "./mcc_generated_files/LINDrivers/lin_slave.h"
...
const uint16_t temperatureLUT[] = {
    ...
    118, 120, 123, 125, 128, 131, 133, 136, 139, 142,
    145, 148, 151, 155, 158, 161, 165, 168, 172, 176,
    179, 183, 187, 191, 195, 200, 204, 208, 213, 217,
    222, 227, 232, 236, 241, 247, 252, 257, 263, 268,
    274, 279, 285, 291, 297, 303, 310, 316, 322, 329,
    336, 342, 349, 356, 363, 370, 377, 385, 392, 400,
    407, 415, 423, 431, 439, 447, 455, 464, 472, 480,
    489, 497, 506, 514, 523, 532, 540, 549, 558, 567,
    575, 584, 593, 601, 610, 619, 628, 636, 645, 654,
    662, 671, 679, 688, 696, 704, 712, 721, 729, 736,
    744, 752, 760, 767, 775, 782, 789, 797, 804, 810,
    817, 824, 830, 837, 843, 849, 855, 861, 867, 872,
    878, 883, 888, 893, 898, 903, 908, 912, 917, 921,
    925, 929, 933, 937, 941, 944, 948, 951, 954, 957,
    960, 963, 966, 968, 971, 973, 976, 978, 980, 982,
    984, 986, 988, 990, 992, 993, 995, 996, 998, 999
};



uint16_t a1_coef = 65326;
uint8_t b1_coef = 105;
uint16_t current2counts_squared = 670;
uint8_t dac_i_hw_trip = I_HW_TRIP_33A;
uint16_t factor_rdson_rthjs = 421;
uint16_t factor_rdson_rthsa = 778;
uint16_t isense_max = 44;
uint8_t reduced_drive_time = 0;
uint16_t tcc_sample_time = 1000;
uint8_t tj_limit = 175;
...
void EFuseInit(void)
{
    ...
    DAC1CON1bits.DAC1R = dac_i_hw_trip;
...
void EFuse(void)
{
    ...
    if(ticks_1ms >= tcc_sample_time)
    {
        ...
        if(CLCDATAbits.MLC4OUT && (outputState == OUTPUT_ON))
        {
        ...
        {
            if((adcISense_curr >= iSenseOffset) && (iSenseOffset < 10))
            {
                adcISense_corr = adcISense_curr - iSenseOffset;
            }
            else
            {
                adcISense_corr = adcISense_curr;
            }
            adcISense_OneMOSFET = adcISense_corr / 2;
            TCC_Curve();
...
# 339 "efuse.c"
    ...
    if(ticks_1ms)
    {
        ...
        if((adcVccSense_prev < 372) && (adcVccSense_curr < 372) && (outputState == OUTPUT_ON))
        {
    ...
    else
    {
        ...
        if((adcTSense_prev < 10) && (adcTSense_curr < 10))
        {
            overTempFault = TEMP_SENSOR_OORL;

        }
        else if((adcTSense_prev > 1013) && (adcTSense_curr > 1013))
        {
            overTempFault = TEMP_SENSOR_OORH;
            CmdOutput(OUTPUT_OFF);
        }
    }
}
...
void CmdOutput(uint8_t enable)
{
    ...
    if (enable)
    {
        ...
        CLC2POLbits.LC2G3POL = 0;
        CLC2POLbits.LC2G3POL = 1;
        CLC2POLbits.LC2G3POL = 0;
        ...
        if(triggerType == TRIGGER_TIMER_OVERFLOW)
        {
            ...
            CLC1GLS3bits.LC1G4D3N = 1;
            CLC1GLS3bits.LC1G4D4T = 1;
...
void TCC_Curve(void)
{
    ...
    Trise_raw_curr = (uint32_t) ((((uint64_t) adcISense_corr * (uint64_t) adcISense_corr) * factor_rdson_rthsa) >> 10);
    Trise_filtered = (uint32_t) ((uint64_t) a1_coef * Trise_filtered + (uint32_t) b1_coef * (Trise_raw_prev + Trise_raw_curr)) >> 16;

    Trise_sa = (int16_t) (Trise_filtered / current2counts_squared);
    Tsink = Tambient + Trise_sa;

    Trise_js = (int16_t) (((((uint32_t) adcISense_OneMOSFET * (uint32_t) adcISense_OneMOSFET) * factor_rdson_rthjs) >> 10) / current2counts_squared);
...
int16_t GetAmbientTemperature(void)
{
    ...
    if (adcTSense_curr > adcTSense_prev)
    {
