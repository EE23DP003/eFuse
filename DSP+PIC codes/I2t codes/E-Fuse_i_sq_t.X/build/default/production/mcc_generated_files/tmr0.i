...
void TMR0_ISR(void) {
    ...
    overcurrent_duration_ms += timer0_duration_ms;
    ...
    sendStringToUART("ADC_value:");
    sendNumberToUART(adcISense_curr);


    i2t_accumulated = calculate_i2t(adcISense_curr, overcurrent_duration_ms);

    sendStringToUART("i2t acc:");
    sendNumberToUART(i2t_accumulated);

    if (i2t_accumulated >= i2t_threshold || adcISense_curr < isense_max) {
        TMR0_StopTimer();


        if (i2t_accumulated >= i2t_threshold) {
            sendStringToUART("trip time:");
            sendNumberToUART(overcurrent_duration_ms);
